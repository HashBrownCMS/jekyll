...
class UISchemaProcessor extends HashBrown.Models.Processor {
    // Getters
    static get name() { return 'uischema.org'; }
    ...
    async check(project, environment, value, schemaId) {
        ...
        // Process array items, if they are structs
            ...
            let schema = await HashBrown.Helpers.SchemaHelper.getSchemaById(project, environment, schemaId, true);
    ...
    async process(project, environment, content, language) {
        ...
        let schema = await HashBrown.Helpers.SchemaHelper.getSchemaById(project, environment, content.schemaId, true);
        ...
        let createdBy = await HashBrown.Helpers.UserHelper.getUserById(meta.createdBy);
        let updatedBy = await HashBrown.Helpers.UserHelper.getUserById(meta.updatedBy);
